...
/**************************************************************************
函数功能：获取ADC引脚中巡线传感器的读数，以此更新距离dist1,dist2,dist3
          以及对当前迷宫状态的判断
**************************************************************************/
void IRGetVal() {
...
void updateState() {
    ...
    if (dist1 >= 30)  
    {
      STATE=LEFT_TMP; // 左转-临时直线动作 判据
    }
    
    else if ((dist1 <= 12.5 && dist2 <= 12.5) || 
              (dist1 <= 11)|| 
            (dist1 <= 9 && dist3 >= 13 && dist3 <= 36)) // 右转动作判据
    {
      STATE=ROTATE_RIGHT;
    }
    
    else if (dist1 <= 11 && dist2 <= 8)  // 倒退动作判据
      STATE=BACK;
      
    else 
      STATE=STRAIGHT; // 直线贴墙动作判据
    
    if (STATE==LEFT_TMP && isPrevState(LEFT_TMP) &&
        countStates(20, ROTATE_RIGHT) < 10) STATE=ROTATE_LEFT; // 180°左转动作判据
    
    if (STATE==ROTATE_RIGHT && isPrevState(ROTATE_RIGHT) && dist1 > 8 && dist1 < 20 && 
        dist2 >= 14 && dist2 <= 35 && dist3 < 16) STATE=STRAIGHT; // 校正，为了避免直线贴墙时误判为右转动作
    if (STATE==ROTATE_RIGHT && countStates(20, ROTATE_RIGHT)>3 
		&& dist1>dist3+4 && dist1>11) STATE=STRAIGHT; // 校正，为了避免右旋过量，提前调整为直线动作
}
